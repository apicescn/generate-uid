...
import static com.xueying.seeker.uid.contants.UIDContants.*;
...
public class SnowflakeIdWorker {
     ...
     * 回拨超时错误
     */
    private static final  String ERROR_CLOCK_BACK = "时间回拨，拒绝为超出%d毫秒生成ID";

    /**
     * 属性的范围
     */
    private static final  String ERROR_ATTR_LIMIT = "%s属性的范围为0-%d";

    /**
     * MSG_UID_PARSE
     */
    private static final  String MSG_UID_PARSE = "{\"UID\":\"%s\",\"timestamp\":\"%s\",\"workerId\":\"%d\","
            + "\"dataCenterId\":\"%d\",\"sequence\":\"%d\"}";
    ...
    private final long maxWorkerId = MINUS_ONE_CONSTANT_LONG ^ (MINUS_ONE_CONSTANT_LONG << workerIdBits);

    /** 支持的最大数据标识id，结果是31 */
    private final long maxDatacenterId = MINUS_ONE_CONSTANT_LONG ^ (MINUS_ONE_CONSTANT_LONG << datacenterIdBits);
    ...
    private final long sequenceMask = MINUS_ONE_CONSTANT_LONG ^ (MINUS_ONE_CONSTANT_LONG << sequenceBits);
    ...
    /** 上次生成ID的时间截 */
    private long lastTimestamp = MINUS_ONE_CONSTANT_LONG;
    ...
    public String parseUID(Long uid) {
        // 总位数
        long totalBits = SIXTY_FOUR_CONSTANT_LONG;
        // 标识
        long signBits = 1L;
        // 时间戳
        long timestampBits = FORTY_ONE_CONSTANT_LONG;
        // 解析Uid：标识 -- 时间戳 -- 数据中心 -- 机器码 --序列
        long sequences = (uid << (totalBits - sequenceBits)) >>> (totalBits - sequenceBits);
        long dataCenterId = (uid << (timestampBits + signBits)) >>> (totalBits - datacenterIdBits);
        long workerIds = (uid << (timestampBits + signBits + datacenterIdBits)) >>> (totalBits - workerIdBits);
        ...
        // 格式化输出
        return String.format(MSG_UID_PARSE, uid, date, workerIds, dataCenterId, sequences);
    }
    ...
    public String parseUID(String uid) {
        ...
        String sequences = uid.substring(sequenceStart, len);
        String workerIds = sequenceStart == 0 ? "0" : uid.substring(workerStart, sequenceStart);
        ...
        Date timeDate = new Date(Long.parseLong(time, TWO_CONSTANT_INT) + twepoch);
        String date = new SimpleDateFormat(DATE_PATTERN_DEFAULT).format(timeDate);
        // 格式化输出
        return String.format(MSG_UID_PARSE, uid, date, Integer.valueOf(workerIds, TWO_CONSTANT_INT),
                Integer.valueOf(dataCenterId, TWO_CONSTANT_INT),
                Integer.valueOf(sequences, TWO_CONSTANT_INT));
    }
