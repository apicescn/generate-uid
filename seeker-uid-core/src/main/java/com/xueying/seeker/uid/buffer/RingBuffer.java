 ...
 */
package com.xueying.seeker.uid.buffer;

import com.xueying.seeker.uid.contants.UIDContants;
import com.xueying.seeker.uid.utils.PaddedAtomicLong;
...
public class RingBuffer {
     ...
     *                      it will trigger padding buffer<br>
     *        Sample: paddingFactor=20, bufferSize=1000 -> threshold=1000 * 20 /100,  
     *        padding buffer will be triggered when tail-cursor<threshold</br>
     */
    public RingBuffer(int bufferSize, int paddingFactor) {
        ...
        Assert.isTrue(paddingFactor > 0 && paddingFactor < UIDContants.HUNDRED_CONSTANT_LONG,
                "RingBuffer size must be positive");
        ...
        this.paddingThreshold = bufferSize * paddingFactor / UIDContants.HUNDRED_CONSTANT_LONG;
    }
     ...
     * 
     * <b>Note that: </b> It is recommended to put UID in a serialize way,
     * cause we once batch generate a series UIDs and put
     ...
     * @return  PaddedAtomicLong数组
     */
    private PaddedAtomicLong[] initFlags(int bufferSize) {
        PaddedAtomicLong[] paddedAtomicLongs = new PaddedAtomicLong[bufferSize];
        for (int i = 0; i < bufferSize; i++) {
            paddedAtomicLongs[i] = new PaddedAtomicLong(CAN_PUT_FLAG);
        }
        
        return paddedAtomicLongs;
    }

    /**
     * Setters
     * @param bufferPaddingExecutor bufferPaddingExecutor
     */
